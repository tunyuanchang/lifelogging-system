...
def fptas_select(items, budget, epsilon):
    ...
    # 2. Scaling
    # -----------------------------
    Vmax = max(it["util"] for it in items)
    K = epsilon * Vmax / n

    for it in items:
        it["v"] = int(it["util"] / K + 1e-6)

    # -----------------------------
    # 3. DP (Pareto states)
    # -----------------------------
    dp = [(0, 0.0)]  # (value, weight)
    parent = {}

    # -----------------------------
    # 4. Pruning (critical)
    # -----------------------------
    def beam_prune(states, beam_width = 300):
        return heapq.nlargest(
            beam_width,
            states,
            key=lambda x: (x[0], -x[1])  # high value, low weight
        )
    ...
    for group in group_list:
        ...
        for v, w in dp:
            for it in group:
                ...
                new_states.append((v2, w2))
                parent[(v2, w2)] = (v, w, it)

        dp = beam_prune(new_states)

    # -----------------------------
    # 6. best state
    # -----------------------------
    best_v, best_w = max(dp, key=lambda x: x[0])
    # print(best_v, best_w)
    ...
    selected.sort(key=lambda x: (x["video_id"], x["seg_id"]))
...
if __name__ == "__main__":
    ...
    if IDX == -1:
        ...
        FILENAME = f"{STORAGE}_{METHOD}_fptas"
    else:
        LIST = os.path.join(IDX_DIR, f"subset_{args.idx}.txt")
        FILENAME = f"{STORAGE}_{METHOD}_fptas_{IDX}"
    ...
    for step, video_id in enumerate(idx_list):
        try:
            ...
            if max_storage > STORAGE:
                ...
                selected, used_storage = fptas_select(all_items, STORAGE, EPSILON)
