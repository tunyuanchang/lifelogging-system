...
def compute_utility_visual(starts, ends, embeddings, method="sim"):
    if method == "sim":
        ...
        return np.array([
            ((embeddings[s:e+1] - embeddings[s:e+1].mean(axis=0)) ** 2)
            .sum(axis=1)
            .mean()
            for s, e in zip(starts, ends)
        ])
...
def greedy_select(items, budget):
    ...
    for it in items:
        ...
        if used >= budget:
            break
    return sorted(selected, key=lambda x: (x["video_id"], x["seg_id"])), used
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--algorithm", type=str, default="gd")
