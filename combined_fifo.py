...
def fifo_select(items, budget, video_rank):
    items = sorted(items, key=lambda x: (video_rank[x["video_id"]], x["seg_id"]), reverse=True)
...
def export_global_data(selected, metadata_map, video_rank):
    ...
    sort_idx = final_df.sort_values(["video_rank", "index"]).index.to_numpy()
    return final_df.iloc[sort_idx].reset_index(drop=True)
    ...
    for step, video_id in enumerate(idx_list):
        ...
        try:
            ...
            if max_storage > STORAGE:
                replacement = True
                selected, used_storage = fifo_select(all_items, STORAGE, vid_rank)
